...
PKI publicKey 替换 & 未启用 PKI 可演示攻击；启用 PKI 后 Host 拒绝。 & 手动执行。\\
附件安全 & 伪装文件、超大文件和路径穿越文件名被拒绝或净化。 & 手动执行。\\
\bottomrule
\end{longtable}

详细步骤已经整理在 phase10-security-tests.md 中。需要服务器和 Wireshark 的实验保留为手动步骤，是因为它们依赖真实网卡、云安全组、Nginx 证书和抓包截图。
...
第四，附件安全仍不等于恶意文件防护。当前系统能检查大小、扩展名、文件头、文件名和缓存目录，但不能替代杀毒、沙箱和复杂文档解析隔离。
