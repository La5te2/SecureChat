...
\begin{itemize}
  ...
  \item GKA v2 使用临时 X25519 公钥和 Host 分发的 room group key。
  \item 成员加入或离开时，Host 轮换 room group key。
  \item PKI 成员身份认证可以绑定 Client 的 join\_room 公钥和 Host 的 group\_key envelope。
...
角色或组件 & 职责 \\
\midrule
Server & 监听 WebSocket 端口，注册房间，维护成员连接状态，校验房间密码摘要，转发 encrypted relay 和 group\_key envelope。Server 不是群成员，不创建群密钥，不参与密钥协商语义。\\
Host & 创建 roomId，是第一个群成员和群管理者。Host 生成 room group key，协调 GKA v2，处理成员加入和离开时的密钥轮换。\\
Client & 加入已有 room，生成临时 X25519 密钥对，参与 GKA v2，使用 room group key 加解密文本和附件。\\
...
RoomRegistry::RoomState & 保存 roomId、房间密码摘要、Host 成员和 Client 成员映射。\\
SignalingServer::ClientState & 保存单个 WebSocket 对应的 roomId、clientId、userId、username、publicKey、role 和坏消息计数。\\
HostSessionCore & Host 本地会话状态，包括 roomId、username、Client 公钥表、当前 group key、PKI identity context 和附件接收状态。\\
ClientSessionCore & Client 本地会话状态，包括临时 X25519 密钥对、当前 group key、PKI identity context 和附件接收状态。\\
...
\begin{itemize}
  ...
  \item group\_key：Host 为目标 Client 单独封装 room group key。
...
Host 为该 Client 生成一次性封装私钥 \(e\)，公钥为：
...
共享秘密不直接作为加密密钥，而是经过基于 HMAC 的密钥派生函数（HMAC-based Key Derivation Function, HKDF）派生包装密钥：

\[
K_W = \operatorname{HKDF\text{-}SHA256}(S,\ \text{salt}=\text{``securechat-gka-v2:''}||roomId,\ \text{info}=\text{``group-key|''}||clientId)
\]

Host 生成 32 字节 room group key \(K_G\)，再使用高级加密标准（Advanced Encryption Standard, AES）的 Galois/Counter Mode（GCM）封装：

\[
(C,T) = \operatorname{AES\text{-}256\text{-}GCM.Enc}(K_W,\ N,\ K_G,\ A)
\]

其中 \(N\) 是随机 nonce，\(A\) 是附加认证数据（Additional Authenticated Data, AAD）。AAD 绑定 roomId 和 targetId，防止 Server 静默修改 group\_key 的目标成员。
...
其中 \(P\) 是明文 JSON 或附件二进制块，\(C\) 是密文，\(T\) 是认证标签。AAD 绑定 roomId、senderId、senderKind 和 targetId：

\[
A = \text{``securechat-relay|''} || roomId || senderId || senderKind || targetId
\]

这意味着攻击者如果修改 roomId、senderId、senderKind 或 targetId，接收端重新构造 AAD 后会导致 GCM 认证失败。
...
Host 发送 group\_key envelope 时签名：

\[
M_G = roomId || targetId || senderId || alg || kdf || ephemeralPublicKey || nonce || ciphertext || tag || identityNonce
\]
...
Client 先验证 Host 证书链、吊销状态和签名，再解封装 group key。这样恶意 Server 不能静默替换 Client public key，也不能静默篡改 group\_key envelope。
...
\subsubsection{GKA v2 群密钥封装}

Host 为每个 Client 单独生成一次性 X25519 密钥对，并派生 wrapping key 封装 room group key。

\begin{lstlisting}[language=C++,caption={secure\_relay.cpp：单成员 group key 封装}]
json encryptGroupKeyForMember(
    ...
    const auto wrapKey = hkdfSha256(
        secret,
        "securechat-gka-v2:" + roomId,
        "group-key|" + targetId);

    return encryptWithAesGcm(plaintext, wrapKey,
        aadForGroupKey(roomId, targetId), {
            {"type", GroupKeyType},
            {"version", 2},
            {"roomId", roomId},
            {"targetId", targetId},
...
文本和附件都封装为 encrypted\_relay。targetId 是明文路由字段，因此必须绑定到 AAD 中，防止恶意 Server 静默改投私发消息。
...
json encryptMessageWithGroupKey(
    ...
    const std::string& targetId,
    const std::vector<unsigned char>& groupKey) {
    // targetId 明文可见，但被 AAD 认证，不能被静默篡改。
...
Host 验证 Client 证书和签名后，会把成员证书指纹作为加密控制消息同步给房间成员。Server 只转发 encrypted relay，不能读取或伪造该控制消息内容；WinUI 只接受 relay metadata 显示发送者是 Host 的 member\_identity 消息，并据此把成员标记为 Verified。

\begin{lstlisting}[language=C++,caption={host\_session\_core.cpp：广播已验证成员指纹}]
...
PKI publicKey 替换 & 篡改 publicKey 或 identity 后 Host/Client 拒绝。 & 手动执行。\\
...
本项目已经完成了从“Host 同时承担信令服务”到“Server/Host/Client 三角色分离”的重构。当前 Server 是不可信信令协调者和密文 relay，不是聊天成员。Host 是第一个群成员和群管理者，Client 是普通成员，二者都使用 room group key 加解密文本和附件。

当前主要安全成果包括：

\begin{itemize}
  \item 移除 WebRTC/DataChannel 数据通路，统一为 TCP WebSocket encrypted relay。
  \item 实现 GKA v2：Client 临时 X25519 public key，Host 分发 room group key。
  \item 实现成员加入/离开后的 group key rotation。
  \item 实现文本、图片、语音、文件 metadata 和 binary chunk 的 AES-256-GCM 加密。
  \item 实现 PKI 成员身份认证，绑定 join\_room publicKey 和 group\_key envelope。
  \item 实现本地证书指纹吊销列表。
...
第一，当前私发是 group key 下的定向投递，而不是独立点对点私聊密钥。Server 只把密文投递给目标成员，但同一房间的其他合法成员理论上仍持有相同 \(K_G\)。如果恶意 Server 复制私发密文给其他成员，其他成员可能具备解密能力。后续可以加入 pairwise key 或 sender key，为私发消息建立独立密钥。

第二，Host 在当前 GKA v2 中仍是密钥协调者。如果 Host 不可信，Host 可以生成、轮换和分发 room group key，因此 Host 在协议中拥有较强权限。要在 Host 不可信场景下提升安全性，需要更复杂的多方 GKA 或 Messaging Layer Security（MLS）一类协议，让群密钥由多方共同贡献，而不是由 Host 单方生成。

第三，Server 仍能观察元数据，包括 roomId、成员在线状态、消息大小、发送频率和转发时序。E2EE 保护内容机密性，但不隐藏通信模式。后续可以考虑 padding、批量发送或混淆转发时序，但会增加带宽和延迟成本。

第四，附件安全仍不等于恶意文件防护。当前系统能检查大小、扩展名、文件头、文件名和缓存目录，WinUI 也默认不自动预览未知成员附件，并在预览前检查图片尺寸、像素数和 WAV 结构；但这些措施不能替代杀毒、沙箱和复杂文档解析隔离。

第五，当前 PKI 吊销使用本地受信任指纹列表。它简单可复现，但不是完整在线证书状态协议（Online Certificate Status Protocol, OCSP）或证书吊销列表（Certificate Revocation List, CRL）体系。正式部署可进一步接入自动化证书签发、吊销分发和密钥轮换流程。

\subsection{对 Host 不可信场景的讨论}

如果 Host 不可信，当前系统不能保证 Host 不读取或不操纵群密钥。原因是 Host 是 GKA v2 的协调者，负责生成 \(K_G\) 并为每个 Client 封装。PKI 可以证明“某个 public key 属于某个成员身份”，mTLS 可以限制“谁能连接入口”，但二者都不能改变 Host 在密钥分发中的权限。

若要对不可信 Host 建立更强安全性，可以考虑：

\begin{itemize}
  \item 多方 DH 或树形群密钥协商，使每个成员都贡献密钥材料。
  \item MLS 协议，使用 TreeKEM 等机制管理群成员变更。
  \item 私发采用独立 pairwise key，避免 Host 或其他成员共享私发密钥。
